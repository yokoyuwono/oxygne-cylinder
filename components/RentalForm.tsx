...
const RentalForm: React.FC<RentalFormProps> = ({ cylinders, members, prices, gasPrices, transactions, onCompleteRental }) => {
    ...
    const handleScanSubmit = async (e?: React.FormEvent) => {
        ...
        try {
            ...
            } else if (data.status !== CylinderStatus.Available && data.status !== CylinderStatus.Delivery) {
                            ...
                            <div className="absolute left-0 right-0 top-full mt-2 bg-white border border-gray-100 rounded-xl shadow-2xl max-h-60 overflow-y-auto z-30">
                                {memberSearchResults.map((m, idx) => (
                                    <div
                                        ...
                                        className={`px-6 py-4 cursor-pointer flex justify-between items-center transition-colors border-b border-gray-50 last:border-0 ${idx === highlightedMemberIdx ? 'bg-indigo-50 text-indigo-900' : 'text-gray-700 hover:bg-gray-50'
                                            }`}
                                    >
                                        <div>
                                ...
                                <div className="absolute left-0 w-full mt-2 bg-white border border-gray-200 rounded-xl shadow-2xl z-20 overflow-hidden">
                                    {serverSuggestions.map((cyl, idx) => (
                                        <div
                                            ...
                                            className={`px-4 py-3 cursor-pointer border-b border-gray-50 flex items-center justify-between ${idx === highlightedCylinderIdx ? 'bg-indigo-50' : 'hover:bg-gray-50'
                                                }`}
                                        >
                                        ...
                                        {memberHeldCylinders.map(cyl => {
                                            ...
                                            return (
                                                <div
                                                    ...
                                                    className={`cursor-pointer p-3 rounded-xl border transition-all relative overflow-hidden flex justify-between items-center ${isSelected
                                                ...
                                                >
                                                    ...
                                                    <div className={`text-[10px] inline-flex items-center gap-1 px-2 py-1 rounded font-medium ${duration.isLongTerm ? 'bg-red-100 text-red-700' : 'bg-gray-100 text-gray-600'
                                                        }`}>
